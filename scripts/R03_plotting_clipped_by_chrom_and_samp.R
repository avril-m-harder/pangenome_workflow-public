...
##### Get sample names #####
incl.beds <- list.files(path = '.', pattern = '_subpath_included_coords.bed')
samps <- gsub('_subpath_included_coords.bed', '', incl.beds)

OUT <- NULL
OUT1 <- NULL
for(samp in samps){
  ## read in coords for retained sequences
  kept <- read.table(paste0(samp,'_subpath_included_coords.bed'))
...
pdf(by.samp.fn, width = 9, height = plot.ht)
bmar <- min(c(20, max(nchar(samps))/1.5))
...
pdf(by.chrom.fn, width = 7, height = plot.ht)
par(mar = c(5, bmar, 1, 1)+0.1)
plot(0,0, col = 'transparent', 
     xlim = c(min(as.numeric(OUT1[,3])), max(as.numeric(OUT1[,3]))),
